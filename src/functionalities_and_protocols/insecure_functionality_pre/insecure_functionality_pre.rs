...
impl InsecureFunctionalityPre {
    ...
    fn generate_random_vole_macs_and_keys<GFVOLE: Clone + Zero + InsecureRandom + GFAddition + GFMultiplyingBit>(
        public_parameter: &PublicParameter,
        delta: &GFVOLE,
    ) -> (BitVec, GFVec<GFVOLE>, GFVec<GFVOLE>) {
        let mut rng = rand::rng();
        let rand_bit_vec = BitVec::from_vec(
        ...
        (rand_bit_vec, vole_mac_vec, vole_key_vec)
    }
