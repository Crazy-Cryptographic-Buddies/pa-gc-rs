...
impl<'a, GF: Clone + Zero + U8ForGF + GFAdd> VerifierInAllInOneVC<'a, GF> {
    pub fn new(public_parameter: &'a PublicParameter, message_len: usize) -> Self {
        Self {
            public_parameter,
    ...
    pub fn reconstruct(&mut self, nabla: &GF,
        ...
        let mut reconstructed_message_vec: Vec<BitVec> = vec![BitVec::zero_vec(self.message_len); 1 << self.public_parameter.tau];
