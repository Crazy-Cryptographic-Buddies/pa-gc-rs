...
pub(crate) struct ProverInProtocolSVOLE<'a, GF: Clone + Zero> {
    prover_in_all_in_one_vc: &'a mut ProverInAllInOneVC<GF>
}

impl<'a, GF: Clone + Zero + GFAdd + U8ForGF> ProverInProtocolSVOLE<'a, GF> {

    pub fn new(prover_in_all_in_one_vc: &'a mut ProverInAllInOneVC<GF>) -> Self {
        Self {
    ...
    pub fn commit(&mut self, seed: &SeedU8x16) -> Hash {
        self.prover_in_all_in_one_vc.commit(seed);
